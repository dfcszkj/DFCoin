...
public class FileUtils {
	 ...
	 * @throws IOException
	 */
	public static void copyFile(String srcFile, String newFileName) {
		try {
			FileInputStream in = new FileInputStream(srcFile);
			String path=newFileName.replaceAll("^(.*)/.*$","$1");
			mkdir(path+"/");
	...
	public static String saveFile(File file){
		...
		String filename=  "/upload/"+DateUtils.getThisYear()+"/"+DateUtils.getThisMonth()+"/"+
				DateUtils.getThisDay()+"/"+time+MathUtils.randomInt(1000,9999)+file.getName().replaceAll(".*\\.",".");
		copyFile(file.getAbsolutePath(),PathKit.getWebRootPath()+filename);
		return  filename;
	}
	/**
	 * 创建目录
